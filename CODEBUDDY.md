...
### Setup & Dependencies
Install PHP dependencies (league/climate, symfony/yaml, phpunit, phpstan, php-cs-fixer):
```bash
composer install
```
...
**CLI layer (`php-src/Cli/`).** `Application` wraps `league/climate`. The single source of truth for every flag is `Options::ALL` in `Options.php` — this one schema drives CLImate argument registration, `--help` rendering, and validation. `main.php` dispatches into four modes (`help`, `info`, `oneshot`, `interactive`) via `Application::getMode()`. Errors are *thrown* (`Cli\Exception`) rather than `exit()`'d, so the entry point (`bin/h3php.php`) can assert behavior in tests; it catches and translates the exception to an exit code.
